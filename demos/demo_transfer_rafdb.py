...
def main(args):
    ...
    for i in tqdm(range(len(testdata))):
        for j in tqdm(range(len(expdata))):
            ...
            # -- expression transfer
            # exp code from image
            exp_name = expdata[j]['imagename']
            exp_images = expdata[j]['image'].to(device)[None,...]
            with torch.no_grad():
                exp_codedict = deca.encode(exp_images)
            # transfer exp code

            id_codedict_copy = id_codedict.copy()
            asym_face = np.random.randint(3)
            #print(asym_face)
            os.makedirs(os.path.join(savefolder, label, name + '_' + exp_name), exist_ok=True)
            for k in range(11):
                id_codedict = id_codedict_copy.copy()
                # id_codedict['pose'][:,3:] = id_codedict['pose'][:,3:] * (1-k/10) + exp_codedict['pose'][:,3:] * (k/10)
                id_codedict['exp'] = id_codedict['exp'] * (1-k/10) + exp_codedict['exp'] * (k/10)
                if k == 0:
                ...
                for vis_name in ['rendered_images']:
                    ...
                    cv2.imwrite(os.path.join(savefolder, label, name + '_' + exp_name, name + '_' + exp_name + '_level_' + str(k) +'.jpg'), util.tensor2image(visdict[vis_name][0]))

                if args.saveObj:
                    deca.save_obj(os.path.join(savefolder, label, name + '_' + exp_name, name + '_' + exp_name + '_level_' + str(k) + '.obj'), transfer_opdict) 

                if args.saveMat:
                    numpy_dict = dict(shape=id_codedict['shape'].cpu().numpy(),exp=id_codedict['exp'].cpu().numpy(),pose=id_codedict['pose'].cpu().numpy())
                    np.save(os.path.join(savefolder, label, name + '_' + exp_name, name + '_' + exp_name + '_level_' + str(k)),numpy_dict) 

            with open(os.path.join(savefolder, 'label_'+ label +'.csv'), 'a', newline='') as file:
                csvFile = csv.writer(file)
                csvFile.writerow([name, exp_name, label])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--image_path', default='TestSamples/examples/7neutral/', type=str,
                        help='path to input image')
    parser.add_argument('-e', '--exp_path', default='TestSamples/examples/1surprise/', type=str, 
    ...
    parser.add_argument('--label', default='1', type=str, 
                        help='label name')
    # rendering option
